...
import {
  ArrowRight, Briefcase, CalendarDays, Check, ChevronLeft, IndianRupee,
  ClipboardList, Eye, EyeOff, GraduationCap, Heart, Home, LayoutDashboard, LogOut, Menu, MessageCircle, UsersRound,
...
const navItems = [
  ...
  { id: 'fundraisers', label: 'Fundraisers', icon: IndianRupee },
...
function Field({ label, hint, ...props }) {
  ...
  const inputProps = isPassword ? { ...props, type: visible ? 'text' : 'password' } : props;
  return <label className="field"><span>{label}</span><div className="field-input-wrap"><input {...inputProps} />{isPassword && <button type="button" className="toggle-password" onClick={(e) => { e.preventDefault(); setVisible((v) => !v); }} tabIndex={-1} aria-label={visible ? 'Hide password' : 'Show password'}>{visible ? <EyeOff size={16} /> : <Eye size={16} />}</button>}</div>{hint && <small>{hint}</small>}</label>;
}
...
function AuthPanel({ mode, setMode, onClose, onAuthenticated }) {
  ...
  const [forgotForm, setForgotForm] = useState({ identifier: '', code: '', new_password: '', confirm_password: '' });
  const [forgotExpires, setForgotExpires] = useState(300);
...
function HomeFeed({ user, goTo, request, onStartChat }) {
        ...
        items.map((story) =>
          story.id === id
            ? {
                ...
                likes_count: story.is_liked ? Math.max(0, story.likes_count - 1) : story.likes_count + 1
              }
            : story
        )
      );
      const result = await request(`/api/v1/stories/${id}/like`, { method: 'POST' });
      setStories((items) =>
        items.map((story) =>
          story.id === id ? { ...story, likes_count: result.likes_count, is_liked: result.liked } : story
        )
      );
    } catch (err) {
...
function Jobs({ user, request, onStartChat }) {
  ...
  const load = useCallback(async () => { setLoading(true); try { const data = await request('/api/v1/jobs'); setJobs(data); if (!selected && data[0]) setSelected(data[0]); setError(''); } catch (err) { setError(err.message); } finally { setLoading(false); } }, [request, selected]); useEffect(() => { load(); }, []);
  const visible = jobs.filter((job) => {
    const matchesSearch = !filters.title || job.title.toLowerCase().includes(filters.title.toLowerCase()) || job.company.toLowerCase().includes(filters.title.toLowerCase());
  ...
  return (
    <div className="page-content">
      ...
      {loading ? (
        ...
        <EmptyState icon={IndianRupee} title="No campaigns yet" text="When a student venture is ready to raise, it will appear here." />
      )}
...
function Chat({ user, request, initialActiveId, clearInitialActiveId }) {
...
export default function App() {
  ...
  const content = {
    home: <HomeFeed user={user} goTo={(next) => { setProfileOpen(false); setPage(next); }} request={request} onStartChat={(authorId) => { request('/api/v1/chat/conversations', { method: 'POST', body: JSON.stringify({ recipient_id: authorId }) }).then((conversation) => { setChatActiveId(conversation.id); }).finally(() => setPage('chat')); }} />,
    directory: <Directory user={user} request={request} onStartChat={(person) => { request('/api/v1/chat/conversations', { method: 'POST', body: JSON.stringify({ recipient_id: person.id }) }).then((conversation) => { setChatActiveId(conversation.id); }).finally(() => setPage('chat')); }} />,
    jobs: <Jobs user={user} request={request} onStartChat={(authorId) => { request('/api/v1/chat/conversations', { method: 'POST', body: JSON.stringify({ recipient_id: authorId }) }).then((conversation) => { setChatActiveId(conversation.id); }).finally(() => setPage('chat')); }} />,
